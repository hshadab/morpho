...
export async function fetchMorphoMarkets(chainId: number = 1): Promise<MorphoMarket[]> {
  try {
    ...
    return markets.map((m) => ({
      ...
      supplyAPY: m.state.supplyApy * 100, // Convert to percentage
      ...
      lltv: parseFloat(m.lltv) / 1e18 * 100, // Convert to percentage
      isLive: true,
    }));
  } catch (error) {
    console.warn('Failed to fetch live Morpho data, using fallback:', error);
